...
- [ ] 1. 결함을 유닛 테스트로 고정
- [ ] 1.1 별칭이 나중에 등장하는 두 케이스를 재현하는 테스트를 추가한다
...
- [ ] 2. 불변 동작을 지키는 테스트를 확인·보강한다
- [ ] 2.1 별칭이 첫 등장인 기존 케이스와 별칭 없는 케이스가 계속 통과하는지 확인한다
...
- [ ] 3. `declare()`가 별칭을 등장 순서와 무관하게 반영하도록 고친다
- [ ] 3.1 대괄호 별칭이 있는 호출마다 라벨을 무조건 다시 쓴다
...
- [ ] 4. 전체 검증
- [ ] 4.1 라이브러리 전체 테스트와 clippy를 통과시킨다
